...
public interface ResultSetExtractor<T> {
	 ...
	 * @param表示要从中提取数据的ResultSet。
	 * 实现不应该关闭它: 它将被调用JdbcTemplate关闭。@ 返回一个任意结果对象，或
	 * {@code null} 如果没有 (在后一种情况下，提取器通常是有状态的)。
	 * @throws SQLException如果遇到SQLException获取列值或导航 (即，不需要捕获SQLException)
*    *  @throws DataAccessException在自定义异常的情况下
	 */
	@Nullable
